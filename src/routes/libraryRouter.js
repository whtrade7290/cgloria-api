...
router.post('/sunday_school_resource', async (req, res) => {
...
router.post('/sunday_school_resource_count', async (req, res) => {
  const { searchWord, board } = req.body
  const count = await totalContentCount(searchWord, board)
  res.json(count)
})

router.post('/sunday_school_resource_detail', async (req, res) => {
...
router.post('/sunday_school_resource_delete', async (req, res) => {
...
router.post('/sunday_school_resource_edit', uploadFields, async (req, res) => {
  ...
  try {
    ...
    if (!result) {
      return res.status(404).json({ error: 'sunday_school_resource not found' })
    }
    res.json(!!result)
  } catch (error) {
    console.error('Error fetching:', error)
    res.status(500).json({ error: 'Error fetching sunday_school_resource' })
  }
})
